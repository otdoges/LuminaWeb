...
const MOCK_STYLES = [
  { id: 'minimalist', name: 'Minimalist', iconType: 'layout', description: 'Clean, simple and focused on content' },
  { id: 'modern', name: 'Bold & Modern', iconType: 'paintBucket', description: 'Vibrant colors and contemporary design elements' },
  { id: 'corporate', name: 'Professional', iconType: 'type', description: 'Refined, corporate-ready aesthetic' },
  { id: 'creative', name: 'Creative', iconType: 'layout', description: 'Artistic and expressive design approach' },
  { id: 'tech', name: 'Tech-Forward', iconType: 'paintBucket', description: 'Modern technology-focused aesthetic' }
];

const MOCK_COLOR_PALETTES = ['#6366F1', '#8B5CF6', '#EC4899', '#10B981', '#F59E0B', '#EF4444', '#06B6D4', '#84CC16'];

const MOCK_SCREENSHOT_URL = 'https://placehold.co/800x600/e2e8f0/1e293b?text=Website+Screenshot';
...
const NewTransformation = () => {
  ...
  useEffect(() => {
    const loadMockData = () => {
      // Map the mock styles data to include icons
      const stylesWithIcons = MOCK_STYLES.map((style) => {
        let icon;
      ...
      setColorPalettes(MOCK_COLOR_PALETTES);
    };
  ...
  const handleUrlSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Mock screenshot capture and analysis
      await mockCaptureScreenshot(url);
      await mockAnalyzeImage(MOCK_SCREENSHOT_URL);
  ...
  const mockCaptureScreenshot = async (siteUrl: string): Promise<ApiResponse> => {
    // Simulate API delay
    ...
    // Save screenshot URL
    setScreenshotUrl(mockResponse.screenshotUrl);
    return mockResponse;
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadData = await mockUploadImage(file);
      await mockAnalyzeImage(uploadData.imageUrl);
  ...
  const mockUploadImage = async (file: File): Promise<ApiResponse> => {
  ...
  const mockAnalyzeImage = async (imageUrl: string | undefined): Promise<ApiResponse> => {
    if (!imageUrl) {
    ...
    logger.info('Analysis results:', mockResponse.analysis);
