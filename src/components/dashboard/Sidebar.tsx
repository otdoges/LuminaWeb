      ...
      <nav className="flex-1 p-4">
        <ul className="space-y-2">
          {menuItems.map(({ id, label, icon: Icon, path }) => (
            <li key={id}>
              <button
                onClick={() => handleMenuClick({ id, label, icon: Icon, path })}
                className={`
                  w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all duration-200 theme-transition
                  ${activeTab === id
              ...
              >
                <Icon 
                  className={`w-5 h-5 ${activeTab === id ? '' : 'text-primary-500'}`}
                  style={activeTab === id ? { color: currentTheme.accent } : {}}
                />
                {label}
              </button>
            </li>
